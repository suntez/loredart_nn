...
class NablaOperator {
  ...
  static List<List<Matrix>> gradients(List<Layer> trainableLayers, Matrix dLoss) {
    ...
    List<List<Matrix>> meanDeltasAndGradients = List<List<Matrix>>.generate(len + 1,
    ...
    if (trainableLayers[len].activatedDerivativeBuffer!.length == 1) {
      ...
      meanDeltasAndGradients[0][0] = (dLoss % trainableLayers[len].activatedDerivativeBuffer![0]);
    }
    else {
      /// With Jacobian:
      ...
      for (int i = 0; i < trainableLayers[len].activatedDerivativeBuffer!.length; i += 1) {
        if (tempEvalMatrix.m == 0 && tempEvalMatrix.n == 0) {
          tempEvalMatrix = (MatrixOperation.hadamardProductToEachColumn(
            trainableLayers[len].activatedDerivativeBuffer![i], dLoss.getColumn(i)
          )).reduceSumByAxis(1);
        }
        else { 
    ...
    meanDeltasAndGradients[0][1] = (meanDeltasAndGradients[0][0] *
      trainableLayers[len].inputDataBuffer!.T)
    ...
    for (int i = len - 1; i >= 0; i -= 1) {
      ...
      var tempSumPrev = trainableLayers[i + 1].w!.T * meanDeltasAndGradients[len - i - 1][0];

      if (trainableLayers[i].activatedDerivativeBuffer!.length == 1) {   // d(activation_function)/d(data)
        meanDeltasAndGradients[len - i][0] = tempSumPrev % trainableLayers[i].activatedDerivativeBuffer![0];
      }
      else {
        // For Jacobian
        Matrix tempEvalMatrix = Matrix.zero(n: 0, m: 0);
        for (int i = 0; i < trainableLayers[i].activatedDerivativeBuffer!.length; i += 1) {
          if (tempEvalMatrix.m == 0 && tempEvalMatrix.n == 0) {
            tempEvalMatrix = (MatrixOperation.hadamardProductToEachColumn(
              trainableLayers[i].activatedDerivativeBuffer![i], tempSumPrev.getColumn(i)
            )).reduceSumByAxis(1);
          }
          else { 
            tempEvalMatrix = MatrixOperation.columnBind(
      ...
      meanDeltasAndGradients[len - i][1] = (meanDeltasAndGradients[len - i][0] * trainableLayers[i].inputDataBuffer!.T)
        .scaled(1 / trainableLayers[i].inputDataBuffer!.m);
      meanDeltasAndGradients[len - i - 1][0] = meanDeltasAndGradients[len - i - 1][0].reduceMeanByAxis(0);
    }
    meanDeltasAndGradients[len][0] = meanDeltasAndGradients[len][0].reduceMeanByAxis(0);
