...
class NablaOperator {
  ...
  static List<List<Matrix>> gradients(
    ...
    List<List<Matrix>> deltasAndGradients = List<List<Matrix>>.generate(len + 1,
    ...
    if (trainableLayers[len].activatedDerivativeBuffer!.isVector) {
      ...
      deltasAndGradients[0][0] =
          dLoss % trainableLayers[len].activatedDerivativeBuffer!;
    } else {
      /// Output [Layer] deltas as sum from i=0 to n-1 of Hadamard product of d(loss)/d(output_i) and d(activation_function_i)/d(data_i)
      deltasAndGradients[0][0] = Matrix.column(List<double>.generate(
          trainableLayers[len].activatedDerivativeBuffer!.n,
          (index) => (trainableLayers[len]
    ...
    deltasAndGradients[0][1] =
        deltasAndGradients[0][0] * trainableLayers[len].inputDataBuffer!.T;

    /// For other [Layer]s gradients is computed as matrix multipliction of deltas and input data of the current [Layer]
    for (int i = len - 1; i >= 0; i -= 1) {
      var tempSumPrev = trainableLayers[i + 1].w!.T *
          deltasAndGradients[len - i - 1]
              [0]; // sum of previous deltas * weights
      if (trainableLayers[i].activatedDerivativeBuffer!.isVector) {
        deltasAndGradients[len - i][0] = tempSumPrev %
            trainableLayers[i]
                .activatedDerivativeBuffer!; // d(activation_function)/d(data)
      } else {
        deltasAndGradients[len - i][0] = Matrix.column(List<double>.generate(
            trainableLayers[i].activatedDerivativeBuffer!.n,
            (index) => (tempSumPrev %
                    trainableLayers[i]
                        .activatedDerivativeBuffer!
                        .getColumn(index))
      ...
      deltasAndGradients[len - i][1] = deltasAndGradients[len - i][0] *
          trainableLayers[i].inputDataBuffer!.T; // delta_i * input_i
    }
