...
class Activation {
  ...
  /// ```
  Activation.sigmoid() {
    function = (Matrix m, [dynamic param]) => m.apply((double x) => 1/(1 + math.exp(-x)));
  ...
  /// ```
  Activation.swish() {
    function = (Matrix m, [dynamic param]) => m.apply((double x) => x/(1 + math.exp(-x)));
    dfunction = (Matrix m, [dynamic param]) {
      final swishMatrix = function(m);
      return swishMatrix + (m.apply((double x) => 1/(1 + math.exp(-x)))%(-swishMatrix.addedScalar(-1)));
    };
    name = 'swish';
  }
  ...
  /// ```
  Activation.softplus() {
    function = (Matrix m, [dynamic param]) => m.apply((double x) => math.log(math.exp(x)+1));
    dfunction = (Matrix m, [dynamic param]) => m.apply((double x) => 1/(1 + math.exp(-x)));
    name = 'softplus';
  }
  ...
  Activation.tanh() {
    ...
    dfunction = (Matrix m, [dynamic param]) => m.apply((double x) => 1/math.pow(cosh(x), 2));
    name = 'tanh';
  }
  ...
  Activation.softmax() {
    function = (Matrix m, [dynamic param]) {
      ...
      return exps.scaled(1/exps.flattenList().reduce((value, element) => value + element));
    };
    dfunction = (Matrix m, [dynamic jacobiMatrix = true]) {
      final softMatrix = function(m);
      if (jacobiMatrix as bool) {
        return Matrix.diag(diag: softMatrix.flattenList()) - softMatrix*softMatrix.T;
      }
  ...
  Activation.linear() {
    ...
    dfunction = (Matrix m, [dynamic param]) => Matrix.zero(n: m.n, m: m.m).addedScalar(1);
    name = 'linear';
  }
  ...
  /// ```  
  Activation.relu() {
    function = (Matrix m, [dynamic param]) => m.apply((double x) => math.max(x,0));
    dfunction = (Matrix m, [dynamic param]) => m.apply((double x) => x > 0 ? 1 : 0);
    name = 'relu';
  }
  ...
  Activation.leakyReLU() {
    function = (Matrix m, [dynamic alpha = 0.1]) => m.apply((double x) => x > 0 ? x : x*(alpha as double));
    dfunction = (Matrix m, [dynamic alpha = 0.1]) => m.apply((double x) => x > 0 ? 1 : (alpha as double));
    name = 'leaky_relu';
  }
  ...
  /// ```  
  Activation.elu() {
    function = (Matrix m, [dynamic alpha = 0.1]) => m.apply((double x) => x > 0 ? x : (alpha as double)*(math.exp(x)-1));
    dfunction = (Matrix m, [dynamic alpha = 0.1]) => m.apply((double x) => x > 0 ? 1 : (alpha as double)*math.exp(x));
    name = 'elu';
  }
