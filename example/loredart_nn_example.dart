...
void main() {
  List<List<List<double>>> data = createTrainData(
    CsvToListConverter().convert(
      File(
        '.csv' //name of the .csv file with data
        ).readAsStringSync(),
  ...
  NeuralNetwork mnistDNN = NeuralNetwork(784,[
  ...
  NeuralNetwork ceMnistDNN = NeuralNetwork(784,[
  ...
  print(hist2); // {mean cross_entropy: 0.23781868465052503, accuracy: 0.9278333333333333}
}
...
List<List<List<double>>> createTrainData(List<List<dynamic>> data) {
  ...
  for (var data1 in data) {
    ...
    x.add(data1.sublist(1).map((e) => double.parse(e.toString())/255).toList());
  }
